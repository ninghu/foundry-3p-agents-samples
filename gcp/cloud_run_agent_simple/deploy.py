...
"""Build and deploy the Cloud Run currency agent (simple)."""
...
PROJECT_KEYS = {"GCP_PROJECT_ID", "PROJECT_ID", "GOOGLE_CLOUD_PROJECT"}
REGION_KEYS = {"GCP_REGION", "REGION", "GOOGLE_CLOUD_REGION"}
...
def ensure_gcloud() -> str:
    ...
    for candidate in ("gcloud.cmd", "gcloud"):
        ...
        if resolved:
            return resolved
    raise RuntimeError(
        "gcloud CLI not found on PATH. Install and configure the Google Cloud SDK first.",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build and deploy the Cloud Run currency agent (simple).",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
        default=resolve_default_env(),
        help="Path to the .env file containing deployment settings.",
    )
    parser.add_argument(
        "--service-name",
        default="gcp-cloud-run-currency-agent",
    ...
    parser.add_argument(
        "--image-tag",
        default=None,
        help="Container image tag (default: timestamp).",
    )
    parser.add_argument(
        "--allow-unauthenticated",
        action="store_true",
        default=True,
        help="Allow unauthenticated invocation of the Cloud Run service (default: on).",
    )
    parser.add_argument(
        "--no-allow-unauthenticated",
        dest="allow_unauthenticated",
        action="store_false",
        help="Disable unauthenticated invocation.",
    )
    parser.add_argument(
        "--extra-env",
        default=None,
        help="Comma-separated KEY=VALUE pairs appended to Cloud Run env vars.",
    )
    ...
    if not project_id or not region:
        raise RuntimeError(
            "Set both GCP project (GCP_PROJECT_ID/PROJECT_ID/GOOGLE_CLOUD_PROJECT) "
            "and region (GCP_REGION/REGION/GOOGLE_CLOUD_REGION) in the env file.",
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to execute gcloud. Ensure the Google Cloud SDK is installed and accessible.",
        ) from exc
